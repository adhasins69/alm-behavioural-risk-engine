...
from src.data_generator import (generate_market_rates, generate_deposit_timeseries,
                                generate_deposit_accounts, generate_loans, synthetic_balance_sheet)
from src.deposit_models import (fit_deposit_beta, fit_error_correction, kaplan_meier,
                                fit_cox_runoff, estimate_core_deposits)
...
from src.analytics import contractual_vs_behavioural_eve, repricing_gap, sensitivity_analysis, monitoring_history
...
def build_data():
    ...
    bs = synthetic_balance_sheet()
    return market, dep_ts, dep_accts, loans, bs
...
def fit_models(dep_ts, dep_accts, loans):
    ...
    prepay_model, prepay_metrics, prepay_coefs, prepay_test = train_prepayment_model(loans)
...
nii_down = calculate_nii(balance_sheet, beta, -200)
validation = validation_summary(beta, prepay_metrics, eve)
contractual_compare = contractual_vs_behavioural_eve(balance_sheet, core, base_cpr)
...
sensitivity = sensitivity_analysis(balance_sheet, beta, core, base_cpr)
monitoring = monitoring_history(beta, core, base_cpr)

st.title("ALM Behavioural Risk Engine")
st.caption("Deposit Behaviour • Loan Prepayment • IRRBB Stress Testing")
st.info("Educational portfolio prototype using fully synthetic banking data. Basel/BNM-inspired; not a production model or regulatory submission.")
...
tabs = st.tabs(["Executive Summary","Deposit Beta","NMD Stability","Loan Prepayment","IRRBB / EVE","NII","Repricing Gap","Sensitivity","Monitoring","Validation","Methodology"])

with tabs[0]:
    st.subheader("Why behavioural modelling matters")
    st.write("Contractual maturities do not fully describe banking-book risk. Non-maturity deposits can remain stable for years, deposit rates can reprice only partially, and borrowers can prepay fixed-rate loans. This engine estimates those behaviours and feeds them into stressed cash flows.")
    c1, c2 = st.columns(2)
    with c1:
        fig = px.bar(eve[eve.scenario != "Baseline"], x="scenario", y="delta_EVE", title="ΔEVE across six MYR scenarios")
        st.plotly_chart(fig, use_container_width=True)
    with c2:
        st.dataframe(core[["segment","stable_proportion","pass_through","core_proportion","behavioural_maturity_years"]].style.format({"stable_proportion":"{:.1%}","pass_through":"{:.1%}","core_proportion":"{:.1%}","behavioural_maturity_years":"{:.2f}"}), use_container_width=True)

with tabs[1]:
    st.subheader("Deposit rate pass-through (deposit beta)")
    st.latex(r"\Delta r_t^D = \alpha + \beta\Delta r_t^M + \epsilon_t")
    st.write("β measures the short-run proportion of a market-rate movement passed through to deposit pricing in the synthetic portfolio.")
    st.dataframe(beta.style.format({"short_run_beta":"{:.3f}","p_value":"{:.4f}","r_squared":"{:.3f}"}), use_container_width=True)
    ...
    st.dataframe(ecm.style.format({"speed_of_adjustment":"{:.3f}","long_run_beta":"{:.3f}","current_change_beta":"{:.3f}","r_squared":"{:.3f}"}), use_container_width=True)
...
with tabs[2]:
    ...
    for seg in dep_accts.segment.unique():
        k = kaplan_meier(dep_accts, seg); k["segment"] = seg; curves.append(k)
    ...
    st.write("Cox proportional-hazards model: hazard ratios above 1 indicate higher runoff hazard, holding other included factors constant.")
    st.dataframe(cox_table.style.format({"coef":"{:.3f}","hazard_ratio":"{:.3f}"}), use_container_width=True)
    st.subheader("Core NMD estimate")
    st.write("Conceptual approximation: Core = Stable × (1 − Pass-through), then constrained by the BNM Exposure Draft caps used here as configurable demonstration parameters.")
    st.dataframe(core.style.format({"balance":"RM {:,.0f}","stable_proportion":"{:.1%}","pass_through":"{:.1%}","core_proportion":"{:.1%}","core_balance":"RM {:,.0f}","behavioural_maturity_years":"{:.2f}"}), use_container_width=True)
...
with tabs[3]:
    ...
    tmp = prepay_test.copy(); tmp["decile"] = pd.qcut(tmp["predicted_monthly_prepay"], 10, labels=False, duplicates="drop")
    cal = tmp.groupby("decile", as_index=False).agg(predicted=("predicted_monthly_prepay","mean"), observed=("prepay_event","mean"))
    fig = go.Figure(); fig.add_trace(go.Scatter(x=cal.predicted,y=cal.observed,mode="lines+markers",name="Observed")); fig.add_trace(go.Scatter(x=[0,cal.predicted.max()],y=[0,cal.predicted.max()],mode="lines",name="Perfect calibration")); fig.update_layout(title="Prepayment calibration by probability decile",xaxis_title="Predicted",yaxis_title="Observed")
    st.plotly_chart(fig, use_container_width=True)

with tabs[4]:
    st.subheader("IRRBB scenario engine")
    st.write("The six MYR shock paths are taken from the September 2025 BNM IRRBB Exposure Draft and are used here only for an educational prototype.")
    ...
    st.subheader("Impact of behavioural modelling")
    st.dataframe(contractual_compare.style.format({"behavioural_EVE":"RM {:,.0f}","behavioural_delta_EVE":"RM {:,.0f}","contractual_EVE":"RM {:,.0f}","contractual_delta_EVE":"RM {:,.0f}"}), use_container_width=True)
...
with tabs[5]:
    ...
    st.write("Deposit-rate changes are scaled by estimated segment deposit betas, while floating-rate assets reprice more fully than fixed-rate assets.")
...
with tabs[6]:
    ...
    st.dataframe(gaps.style.format({"rate_sensitive_assets":"RM {:,.0f}","rate_sensitive_liabilities":"RM {:,.0f}","gap":"RM {:,.0f}","cumulative_gap":"RM {:,.0f}"}), use_container_width=True)

with tabs[7]:
    st.subheader("Behavioural assumption sensitivity")
    st.write("Each case changes one major behavioural assumption while holding the rest broadly constant.")
    ...
    st.dataframe(sensitivity.style.format({"worst_delta_EVE":"RM {:,.0f}","parallel_up_delta_NII":"RM {:,.0f}","EVE_impact_vs_base":"RM {:,.0f}","NII_impact_vs_base":"RM {:,.0f}"}), use_container_width=True)

with tabs[8]:
    st.subheader("Quarterly model monitoring")
    st.write("Synthetic monitoring snapshots illustrate how a risk team could track parameter drift and model performance over time.")
    mlong = monitoring.melt(id_vars=["quarter","status"], value_vars=["retail_transactional_beta","weighted_core_proportion","portfolio_CPR","prepayment_AUC"], var_name="metric", value_name="value")
...
with tabs[9]:
    ...
    st.write("A production framework would add independent validation, richer back-testing, stability monitoring, data-quality controls, approvals and change governance.")
