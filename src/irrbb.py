...
BUCKETS = [("O/N", 0.003), ("1M", 0.042), ("3M", 0.167), ("6M", 0.375), ("9M", 0.625), ("1Y", 0.875),
...
CPR_MULTIPLIER = {"Parallel Up":0.8,"Parallel Down":1.2,"Steepener":0.8,"Flattener":1.2,
                  "Short Rate Up":0.8,"Short Rate Down":1.2}
...
def scenario_curves() -> pd.DataFrame:
    ...
    for name, shocks in MYR_SHOCKS.items():
        frames.append(base.assign(scenario=name, shock_bps=shocks,
    ...
    for _, r in balance_sheet.iterrows():
        ...
        if product == "Fixed-rate retail loans":
            ...
            for yr in range(1, n + 1):
                sched = amount / n
                prep = min(max(rem - sched, 0) * cpr, rem)
...
def calculate_eve(balance_sheet: pd.DataFrame, core_df: pd.DataFrame,
                  base_cpr: float, tier1_capital: float = 2.2e9) -> pd.DataFrame:
    ...
    for scenario in ["Baseline"] + list(MYR_SHOCKS):
        ...
        for _, row in cf.iterrows():
            rate = _nearest_rate(curve, row["years"])
            pv += row["sign"] * row["cashflow"] * float(discount_factor(rate, row["years"]))
        results.append({"scenario": scenario, "EVE": pv})
...
def calculate_nii(balance_sheet: pd.DataFrame, beta_summary: pd.DataFrame, shock_bps: float = 0.0) -> float:
    ...
    for _, r in balance_sheet.iterrows():
        ...
        if r["side"] == "Asset":
            ...
            if product == "Retail transactional NMD": beta = betas.get("Retail transactional", .2)
            elif product == "Retail non-transactional NMD": beta = betas.get("Retail non-transactional", .45)
            elif product == "Wholesale NMD": beta = betas.get("Wholesale", .75)
            else: beta = 0.65
            expense += amount * max(rate + beta * shock_pct_points, 0) / 100
    return income - expense


def duration_convexity_approx(price: float, modified_duration: float,
                              convexity: float, delta_y: float) -> tuple[float,float]:
    duration_only = price * (-modified_duration * delta_y)
    duration_convexity = price * (-modified_duration * delta_y + 0.5 * convexity * delta_y**2)
    return duration_only, duration_convexity
